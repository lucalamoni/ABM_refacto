...
#                    if np.any(self.CMscore<0.00):
#                        print('NEGS IN MATTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT')
#                        print(self.name)
#                        print(np.around(self.mat))
                ...
                if np.remainder(n,self.iSave)==0:
                    for m in range(0,self.nw):
                        data = {'MigrationNumber' : numMig,
                                ...
                                'song_memoryMat':self.Agents[m].Memory_Mat,
...
#            print(a.location)
    ...
    #Caclulate the intensity factors between pairs of agents. 
...
# function to chose randomly the ids of the agents that will emigrate to the hosting population feeding grounds and/or breeding grounds
    def choose_immigrants(self, n_of_imm, population_size):
        meta_pop = int(population_size * 0.5)
    ...
    def weightedGrammar(self,agentNumber,Agents,modelMode,CMscore,distMat,intensityMatrix): #noveltyScore is in reality noveltyScores
        ...
        for a in Agents: 
            ...
            if a.name != self.name and distMat[self.name,a.name] < 50:
                ...
                if (modelMode == 'novelty') or modelMode == ('weightedEditsN'):
                    scalar = 1 - (intensityMatrix[self.name,a.name]*self.noveltyValues[a.name]) #NOTE PERFORMANCE COULD BE IMPROVED BY MOVING NOVELTY CALCULATIONS OUT OF AGENT LOOP
                    scalar2 = (intensityMatrix[self.name,a.name]*self.noveltyValues[a.name])
                    self.mat = (self.mat*(scalar) +  G2*scalar2)
                ...
                if (modelMode == 'revolution'):
                    ...
                    #multiply intFact and CM
                    scalar = 1 - (intensityMatrix[self.name,a.name]*CMscore[self.name]) #official
                    scalar2 = (intensityMatrix[self.name,a.name]*CMscore[self.name])
                    self.mat = (self.mat*(scalar) +  G2*scalar2)
